...
export class SomReannotationRequiredError extends Error {
...
export class PageSoMHandler {
  ...
  async updateSom(includeOffscreen: boolean = false, includeDisabled: boolean = false): Promise<number> {
    ...
    const elements = await this.page.evaluate((params: { includeOffscreen: boolean, includeDisabled: boolean }) => {
      ...
      querySelectorAllDeep('canvas').forEach((canvas: any) => {
       ...
       */
      function mapTagToPlaywrightRole(tag: string, elementType?: string): string {
        ...
        if (tag === 'input') {
          ...
          if (elementType === 'submit') return 'button';
  ...
  private async resolveSomTarget(
    elementRef: string,
    expected: SomElement
  ): Promise<{ index: number; duplicateCount: number; candidates: Array<Record<string, unknown>> }> {
    const duplicates = await this.page.evaluate((ref: string) => {
    ...
    const expectedClassTokens = new Set(
      (expected.className || '')
        .split(/\s+/)
    ...
    const scored = duplicates.map((candidate) => {
      const candidateClassTokens = new Set(
        (candidate.className as string)
          .split(/\s+/)
          .map((token) => token.trim())
          .filter(Boolean),
      );
      ...
      const candidateText = (candidate.text as string).trim();
      if (expectedText) {
      ...
      const candidateAria = (candidate.ariaLabel as string).trim();
      if (expectedAria && candidateAria === expectedAria) {
        score += 2;
      }

      const bbox = candidate.bbox as { x: number; y: number; width: number; height: number };
    ...
    if (!best || best.score <= 0) {
      throw new SomReannotationRequiredError(
        `Duplicate SoM id "${elementRef}" no longer matches original element`,
        {
          elementRef,
          expected,
          candidates: scored,
    ...
    if (duplicateCount > 1) {
      this.logger?.(
        `[PageSoMHandler] Resolved duplicate tc-som-id="${elementRef}" with ${duplicateCount} candidates (chosen index=${best.index}, score=${best.score})`,
        'log',
      );
    }

    return {
      index: best.index as number,
      duplicateCount,
      candidates: scored,
    };
  }
  ...
  async runCommand(
    ...
    this.logger?.(
      `[PageSoMHandler] Executing ${command.action} on element ${command.elementRef}`,
      'log',
    );
   ...
   * Based on W3C ARIA specifications and Playwright's supported roles
   */
  private mapToPlaywrightRole(roleOrTag: string, elementType?: string): string {
  ...
  private buildLocatorFromTypedSelector(typedSelector: TypedSelector): any {
    ...
    switch (typedSelector.type) {
      ...
      case 'placeholder':
        ...
        break;
      case 'text':
        locator = base.getByText(
          typedSelector.value,
          typedSelector.exact ? { exact: true } : undefined
        );
        break;
      case 'title':
   ...
   */
  private async executeCanvasRelativeAction(
    command: SomCommand,
    element: SomElement
  ): Promise<SemanticCommandResult> {
    ...
    const semanticSelectors = this.generateSemanticSelectors(element);
    let canvasSelector: TypedSelector;
    ...
    try {
      switch (command.action) {
        case InteractionAction.CLICK: {
          ...
          const options: any = { position: { x, y }, timeout };
          if (command.force) options.force = true;
        ...
        case InteractionAction.RIGHT_CLICK: {
          const options: any = { position: { x, y }, button: 'right', timeout };
          if (command.force) options.force = true;
        ...
        case InteractionAction.TYPE: {
          ...
          if (command.action === InteractionAction.FILL) {
   ...
   */
  private escapeStringValue(value: string | undefined | null): string {
    if (value == null) {
      return '';
    }
    return String(value)
